...
def preprocess_text(text: str) -> list[str]:
    """Lowercase → tokenize → remove non-alpha & stopwords → lemmatize."""
...
def clean_text(text: str) -> str:
    """Remove URLs, slide numbers, bullet symbols, fix spacing, and split concatenated words."""
    text = re.sub(r'https?://\S+', '', text)                      # remove URLs
    text = re.sub(r'\b\d+/\d+\b', '', text)                       # remove slide numbers like 26/36
    text = re.sub(r'[•\uf0b7\uf0a7◦▪▸●○□■–—]', ' ', text)        # replace bullet/dash symbols with space
    text = re.sub(r'([a-z])([A-Z])', r'\1 \2', text)              # fix camelCase splits e.g. "aMarkov" → "a Markov"
    text = re.sub(r'([A-Za-z])(\d)', r'\1 \2', text)              # split letters glued to numbers
    text = re.sub(r'(\d)([A-Za-z])', r'\1 \2', text)              # split numbers glued to letters
    text = re.sub(r'([a-z])(E\.g\.|I\.e\.)', r'\1 \2', text)      # fix "textE.g." → "text E.g."
    text = re.sub(r'[^\x00-\x7F]+', ' ', text)                    # remove non-ASCII characters (math symbols etc.)
...
def extract_text_from_pdf(pdf_path: str) -> list[tuple[int, str]]:
    """
    Extract text from a PDF by rendering each page as an image and running OCR.
...
def chunk_text_with_pages(
    ...
    Returns a list of (chunk_text, page_number) tuples so every chunk
    knows which real PDF page it came from.
    """
    results = []
...
def build_tfidf_vectorizer(texts: list[str]) -> TfidfVectorizer:
    """Fit a TF-IDF vectorizer on the given list of texts."""
...
# ─────────────────────────────────────────────────────────────────────────────
# 4. ChromaDB  —  store / retrieve raw TF-IDF vectors
# ─────────────────────────────────────────────────────────────────────────────
def get_chroma_collection(persist_dir: str = CHROMA_PATH):
    """Return (client, collection). ChromaDB embedding is disabled; we supply our own."""
...
def index_chunks(
    ...
    vectors:  np.ndarray,
    pdf_name: str,
    pages:    list[int],      # ← real page number for each chunk
) -> None:
    """Store pre-computed TF-IDF vectors + raw text into ChromaDB."""
    ids       = [str(uuid.uuid4()) for _ in chunks]
    metadatas = [
        {
            "source":      pdf_name,
            "page_number": pages[i],        # ← real PDF page number
            "text":        chunks[i][:500],
        }
...
# ─────────────────────────────────────────────────────────────────────────────
# 5. Build index: extract PDFs → fit TF-IDF → store vectors in ChromaDB
# ─────────────────────────────────────────────────────────────────────────────
def build_index(pdf_files: list[str], collection) -> TfidfVectorizer:
    """
    Two-pass pipeline:
      Pass 1 – collect all chunks (with their real page numbers) from every new PDF.
      Pass 2 – fit TF-IDF on ALL chunks, vectorize, store in ChromaDB.

    Returns the fitted vectorizer (also saved to disk).
    """
    ...
    # ── Pass 1: extract, chunk, and track page numbers ────────────────────────
    all_chunks    = []   # str  — chunk text
    all_pages     = []   # int  — real PDF page number for each chunk
    chunk_sources = []   # str  — which PDF each chunk came from
    ...
    for pdf_path in new_pdf_files:
        ...
        chunks_with_pages = chunk_text_with_pages(pages) # [(chunk_text, page_num), ...]
    ...
    # ── Pass 2: fit TF-IDF on all new chunks, then vectorize ─────────────────
    print(f"\n  [tfidf] Fitting TF-IDF on {len(all_chunks)} chunks ...")
    vectorizer   = build_tfidf_vectorizer(all_chunks)
    tfidf_matrix = vectorizer.transform(all_chunks)      # sparse (n_chunks × vocab)
    save_vectorizer(vectorizer)
    print(f"  [tfidf] Vocabulary size: {len(vectorizer.vocabulary_)} terms")

    # ── Store each PDF's chunks with their vectors and page numbers ───────────
    for pdf_path in new_pdf_files:
        pdf_name = os.path.basename(pdf_path)
...
def main() -> None:
    ...
    if not pdf_files:
        ...
        print(f"Usage: python pdf_search_chromadb.py [pdf_folder]")
        sys.exit(1)
